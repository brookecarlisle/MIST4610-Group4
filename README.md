...
Our model is based on the structure of a hypothetical Cricket Club. The MembershipManagement entity is representative of the club’s members (players) as well as operations it takes to run a club (FinancialManagement and EventAndTournamentManagement). The MembershipManagement table consists of the Unique memberID as the primary key, and the different Financial transactions made by members as the foreign key.  The relationship between MembershipManagement and PlayerProfiles is represented by the one to one relationship we have placed between the two entities. FinancialManagement and MembershipManagement are represented by a one-to-many relationship as one member may have many financial transactions with their one club. The many-to-many relationship between MembershipManagement and EventandTournamentManagement creates the associative entity of PlayerHistory which allows us to see which members participated in which tournaments. 

Going up from MembershipManagement to PlayerProfiles, we have one branch that stems from player profiles and it is a many-to-many relationship with teamManagement. The many-to-many relationship creates an associative entity which shows the Roster for each team (using teamID) and playerID. TeamManagement also connects with CoachingandDeveoplement, with a one-to-one relationship as each team can only have one head coach. We have a one-to-one recursive relationship in the CoachingandDevelopment entity which represents the assistant coaches. Lastly, teamManagement has a one-to-many relationship with fanEngagement as one team can have many different fans, but each fan can only have one team. 
